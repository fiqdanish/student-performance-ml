...
features = [col for col in stud_performance.columns if ('_encoded' in col or '_binned' in col) and col != 'Access_to_Resources_encoded_o']
target = 'Access_to_Resources_encoded_o'
...
print(stud_performance.corr(numeric_only=True)['Access_to_Resources_encoded_o'].sort_values(ascending=False)) # Display correlation with target variable
...
print(stud_performance.corr(numeric_only=True)['Access_to_Resources_encoded_o'].sort_values(ascending=False))
